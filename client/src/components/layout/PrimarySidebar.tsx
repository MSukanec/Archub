...
export default function PrimarySidebar() {
  ...
  return (
    <div className="w-[45px] bg-surface border-r border-border flex flex-col items-center py-4 space-y-2">
      {/* Logo */}
      <div className="mb-6">
      ...
      {bottomNavigationItems.map(({ section, icon: Icon, label }) => {
        ...
        return (
          <button
            ...
            onClick={() => setSection(section)}
            className={cn(
              "w-11 h-11 rounded-lg flex items-center justify-center transition-all duration-200 mb-2",
              currentSection === section
                ? "text-primary bg-primary/10"
                : "text-muted-foreground hover:text-foreground hover:bg-surface/60"
            )}
      ...
      <button
        ...
        className="w-11 h-11 rounded-lg flex items-center justify-center transition-all duration-200 mb-2 text-muted-foreground hover:text-foreground hover:bg-surface/60 relative"
